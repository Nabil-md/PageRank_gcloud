...
INIT = Pig.compile("""
A = LOAD 'gs://bucket_nabil_mhd/small_page_links.nt' using PigStorage(' ') as (url:chararray, p:chararray, link:chararray);
B = GROUP A by url;                                                                                  
...
params = { 'd': '0.5', 'docs_in': 'gs://bucket_nabil_mhd/out/pagerank_data_simple' }
...
for i in range(3):
   out = "gs://bucket_nabil_mhd/out/pagerank_data_" + str(i + 1)
   params["docs_out"] = out
   Pig.fs("rmr " + out)
